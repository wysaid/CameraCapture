...
使用 CameraCapture (ccap) 库的功能强大的命令行界面工具。

[English](./README.md) | [中文](./README.zh-CN.md)

## 概述

`ccap` 命令行工具为相机操作提供了全面的命令行界面，包括设备枚举、帧捕获、格式转换和可选的实时预览。它专为自动化、脚本编写和快速测试相机功能而设计。
...
- **YUV 操作**：直接 YUV 捕获和 YUV 转图像
- **实时预览**：基于 OpenGL 的预览窗口（使用 GLFW 支持构建时）
- **自动化友好**：专为脚本和 CI/CD 流程设计
...
可执行文件将位于 `build/` 目录（或 `build/Debug`、`build/Release`，取决于构建配置）。
...
| `-i, --device-info [INDEX]` | 显示 INDEX 位置设备的详细能力<br>如果省略 INDEX，则显示所有设备的信息 |
...
| `--save-yuv` | - | 将帧保存为原始 YUV 数据，而不是转换为 BMP |

### 预览选项

这些选项仅在使用 GLFW 支持构建时可用（`CCAP_CLI_WITH_GLFW=ON`）。
...
| `--convert-output FILE` | 转换图像的输出文件路径 |

## 支持的格式

### RGB/BGR 格式

- `rgb24` - 24 位 RGB（每通道 8 位）
- `bgr24` - 24 位 BGR（每通道 8 位）
- `rgba32` - 32 位 RGBA（每通道 8 位 + alpha）
- `bgra32` - 32 位 BGRA（每通道 8 位 + alpha）
...
示例：`captures/frame_1640000000000_0.bmp`
...
- [主 ccap 文档](../README.zh-CN.md) - CameraCapture 库概述
- [CMake 构建选项](../docs/CMAKE_OPTIONS.md) - 构建配置详情
- [C 接口文档](../docs/C_Interface.md) - C API 参考
- [示例](../examples/) - 使用库的代码示例
