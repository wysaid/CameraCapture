...
TEST_F(AccuracyTest, YUVToRGB_AgainstReference)
{
    ...
    for (const auto& yuv : test_values)
    {
        ...
        // Test BT.601 Video Range
        {
            ...
            EXPECT_NEAR(r_impl, r_ref, tolerance_)
                << "BT601v Red mismatch for YUV(" << y << "," << u << "," << v << ")";
            EXPECT_NEAR(g_impl, g_ref, tolerance_)
                << "BT601v Green mismatch for YUV(" << y << "," << u << "," << v << ")";
            EXPECT_NEAR(b_impl, b_ref, tolerance_)
                << "BT601v Blue mismatch for YUV(" << y << "," << u << "," << v << ")";
        }
        ...
        // Test BT.709 Video Range
        {
            ...
            EXPECT_NEAR(r_impl, r_ref, tolerance_)
                << "BT709v Red mismatch for YUV(" << y << "," << u << "," << v << ")";
            EXPECT_NEAR(g_impl, g_ref, tolerance_)
                << "BT709v Green mismatch for YUV(" << y << "," << u << "," << v << ")";
            EXPECT_NEAR(b_impl, b_ref, tolerance_)
                << "BT709v Blue mismatch for YUV(" << y << "," << u << "," << v << ")";
        }
        ...
        // Test BT.601 Full Range
        {
            ...
            EXPECT_NEAR(r_impl, r_ref, tolerance_)
                << "BT601f Red mismatch for YUV(" << y << "," << u << "," << v << ")";
            EXPECT_NEAR(g_impl, g_ref, tolerance_)
                << "BT601f Green mismatch for YUV(" << y << "," << u << "," << v << ")";
            EXPECT_NEAR(b_impl, b_ref, tolerance_)
                << "BT601f Blue mismatch for YUV(" << y << "," << u << "," << v << ")";
        }
        ...
        // Test BT.709 Full Range
        {
            ...
            EXPECT_NEAR(r_impl, r_ref, tolerance_)
                << "BT709f Red mismatch for YUV(" << y << "," << u << "," << v << ")";
            EXPECT_NEAR(g_impl, g_ref, tolerance_)
                << "BT709f Green mismatch for YUV(" << y << "," << u << "," << v << ")";
            EXPECT_NEAR(b_impl, b_ref, tolerance_)
                << "BT709f Blue mismatch for YUV(" << y << "," << u << "," << v << ")";
        }
    }
}
...
TEST_F(AccuracyTest, ColorShuffle_Roundtrip)
{
    ...
    ccap::bgraToRgba(original.data(), original.stride(),
                     intermediate.data(), intermediate.stride(),
                     width_, height_);

    ccap::rgbaToBgra(intermediate.data(), intermediate.stride(),
    ...
    EXPECT_TRUE(PixelTestUtils::compareImages(
        ...
        width_, height_, 4,
        original.stride(), result.stride(), 0))
...
TEST_F(AccuracyTest, RGB_RGBA_Roundtrip)
{
    ...
    // Should be identical
    EXPECT_TRUE(PixelTestUtils::compareImages(
        rgb_original.data(), rgb_result.data(),
        width_, height_, 3,
        rgb_original.stride(), rgb_result.stride(), 0))
...
TEST_F(AccuracyTest, NV12_I420_Consistency)
{
    ...
    ccap::nv12ToRgb24(nv12_img.y_data(), nv12_img.y_stride(),
                      ...
                      rgb_from_nv12.data(), rgb_from_nv12.stride(),
                      width_, height_);

    ccap::i420ToRgb24(i420_img.y_data(), i420_img.y_stride(),
                      i420_img.u_data(), i420_img.uv_stride(),
                      i420_img.v_data(), i420_img.uv_stride(),
                      rgb_from_i420.data(), rgb_from_i420.stride(),
                      width_, height_);

    // Results should be very similar (allowing for minor implementation differences)
    EXPECT_TRUE(PixelTestUtils::compareImages(
        rgb_from_nv12.data(), rgb_from_i420.data(),
        width_, height_, 3,
        rgb_from_nv12.stride(), rgb_from_i420.stride(), 1))
...
TEST_F(AccuracyTest, RGB_BGR_Relationship)
{
    ...
    ccap::rgbaToRgb(rgba_src.data(), rgba_src.stride(),
...
TEST_F(AccuracyTest, BoundaryValues)
{
    ...
    for (const auto& test : boundary_tests)
    {
        ...
        EXPECT_TRUE(PixelTestUtils::isValidRGB(r, g, b))
            << "BT601v boundary test failed for " << test.description
            << " YUV(" << test.y << "," << test.u << "," << test.v << ")"
            << " -> RGB(" << r << "," << g << "," << b << ")";

        ccap::yuv2rgb709v(test.y, test.u, test.v, r, g, b);
        EXPECT_TRUE(PixelTestUtils::isValidRGB(r, g, b))
            << "BT709v boundary test failed for " << test.description;

        ccap::yuv2rgb601f(test.y, test.u, test.v, r, g, b);
        EXPECT_TRUE(PixelTestUtils::isValidRGB(r, g, b))
            << "BT601f boundary test failed for " << test.description;

        ccap::yuv2rgb709f(test.y, test.u, test.v, r, g, b);
        EXPECT_TRUE(PixelTestUtils::isValidRGB(r, g, b))
            << "BT709f boundary test failed for " << test.description;
    }
}
...
TEST_F(AccuracyTest, StatisticalAccuracy)
{
    ...
    double mse = PixelTestUtils::calculateMSE(
        rgb_dst.data(), reference_rgb.data(),
        large_width, large_height, 3,
        rgb_dst.stride(), reference_rgb.stride());
    ...
    for (int y = 0; y < large_height; ++y)
    {
        ...
        for (int x = 0; x < large_width; ++x)
        {
            ...
            for (int c = 0; c < 3; ++c)
            {
                int diff = std::abs(static_cast<int>(impl_row[x * 3 + c]) -
                                    static_cast<int>(ref_row[x * 3 + c]));
                if (diff > tolerance_)
                {
    ...
    std::cout << "[ACCURACY] Pixels within tolerance: "
              << accuracy_percentage << "%" << std::endl;

    EXPECT_GT(accuracy_percentage, 95.0)
        << "At least 95% of pixels should be within tolerance";
}
