...
public:
     ...
     */
    static void runWithBothImplementations(
        ...
        if (hasAVX2Support) {
            ...
            ASSERT_NO_THROW(test_func()) 
                << test_name << " failed with AVX2 enabled";
            ...
            ASSERT_NO_THROW(test_func()) 
                << test_name << " failed with AVX2 disabled (CPU implementation)";
            
            // Restore AVX2 state
            ccap::disableAVX2(false);
            
            std::cout << "[INFO] " << test_name 
                      << " passed for both AVX2 and CPU implementations" << std::endl;
        } else {
            ...
            ASSERT_NO_THROW(test_func()) 
                << test_name << " failed with CPU implementation";
            
            std::cout << "[INFO] " << test_name 
                      << " passed for CPU implementation (AVX2 not supported)" << std::endl;
        }
    }
    ...
    static void runComparisonTest(
        ...
        const std::string& test_name = "Comparison Test"
    ) {
        bool hasAVX2Support = ccap::hasAVX2();
        ...
        if (hasAVX2Support) {
            ...
            // Compare results
            EXPECT_EQ(avx2_result, cpu_result)
                << test_name << ": AVX2 and CPU implementations should produce identical results";
            
            // Restore AVX2 state
            ccap::disableAVX2(false);
            
            std::cout << "[INFO] " << test_name 
                      << " - AVX2 and CPU implementations produce identical results" << std::endl;
        } else {
            ...
            std::cout << "[INFO] " << test_name 
                      << " completed (AVX2 not supported, tested CPU implementation only)" << std::endl;
        }
    }
     ...
     */
    static void runImageComparisonTest(
        std::function<TestImage()> test_func,
        const std::string& test_name = "Image Comparison Test",
        int tolerance = 0
    ) {
        bool hasAVX2Support = ccap::hasAVX2();
        ...
        if (hasAVX2Support) {
            ...
            // Compare images
            ASSERT_EQ(avx2_result.width(), cpu_result.width())
                << test_name << ": Image dimensions should match";
            ASSERT_EQ(avx2_result.height(), cpu_result.height())
                << test_name << ": Image dimensions should match";
            ASSERT_EQ(avx2_result.channels(), cpu_result.channels())
                << test_name << ": Image channels should match";
            
            // Compare pixel data
            bool images_match = PixelTestUtils::compareImages(
                avx2_result.data(), cpu_result.data(),
                avx2_result.width(), avx2_result.height(), avx2_result.channels(),
                avx2_result.stride(), cpu_result.stride(), tolerance
            );
            ...
            EXPECT_TRUE(images_match)
                << test_name << ": AVX2 and CPU implementations should produce identical images (tolerance: " 
                << tolerance << ")";
            ...
            std::cout << "[INFO] " << test_name 
                      << " - AVX2 and CPU implementations produce identical images" << std::endl;
        } else {
            // Just run the test once if AVX2 is not supported
 ...
 */
#define RUN_DUAL_IMPLEMENTATION_TEST(test_func) \
    AVX2TestRunner::runWithBothImplementations([&]() { test_func; }, #test_func)
