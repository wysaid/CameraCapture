...
TEST_F(YUVFormatConversionTest, NV12_To_BGR24_BT601_VideoRange)
{
    ...
    // Convert NV12 -> BGR24
    ccap::nv12ToBgr24(yuv_img.y_data(), yuv_img.y_stride(), yuv_img.uv_data(), yuv_img.uv_stride(), bgr_img.data(), bgr_img.stride(),
    ...
    for (int y = 0; y < height_; ++y) {
        ...
        for (int x = 0; x < width_; ++x) {
            ...
            EXPECT_TRUE(PixelTestUtils::isValidRGB(r, g, b)) << "Invalid RGB at (" << x << "," << y << "): RGB(" << static_cast<int>(r)
...
TEST_F(YUVFormatConversionTest, NV12_To_RGB24_BT601_VideoRange)
{
    ...
    ccap::nv12ToRgb24(yuv_img.y_data(), yuv_img.y_stride(), yuv_img.uv_data(), yuv_img.uv_stride(), rgb_img.data(), rgb_img.stride(),
                      width_, height_, ccap::ConvertFlag::Default);
...
TEST_F(YUVFormatConversionTest, NV12_To_BGRA32_BT709_VideoRange)
{
    ...
    ccap::nv12ToBgra32(yuv_img.y_data(), yuv_img.y_stride(), yuv_img.uv_data(), yuv_img.uv_stride(), bgra_img.data(), bgra_img.stride(),
...
TEST_F(YUVFormatConversionTest, NV12_To_RGBA32_FullRange)
{
    ...
    ccap::nv12ToRgba32(yuv_img.y_data(), yuv_img.y_stride(), yuv_img.uv_data(), yuv_img.uv_stride(), rgba_img.data(), rgba_img.stride(),
...
TEST_F(YUVFormatConversionTest, I420_To_BGR24_BT601_VideoRange)
{
    ...
    ccap::i420ToBgr24(yuv_img.y_data(), yuv_img.y_stride(), yuv_img.u_data(), yuv_img.uv_stride(), yuv_img.v_data(), yuv_img.uv_stride(),
                      bgr_img.data(), bgr_img.stride(), width_, height_, ccap::ConvertFlag::Default);
...
TEST_F(YUVFormatConversionTest, I420_To_RGB24_BT709_FullRange)
{
    ...
    ccap::i420ToRgb24(yuv_img.y_data(), yuv_img.y_stride(), yuv_img.u_data(), yuv_img.uv_stride(), yuv_img.v_data(), yuv_img.uv_stride(),
                      rgb_img.data(), rgb_img.stride(), width_, height_, ccap::ConvertFlag::BT709 | ccap::ConvertFlag::FullRange);
    ...
    bool has_variation = (first_pixel[0] != last_pixel[0]) || (first_pixel[1] != last_pixel[1]) || (first_pixel[2] != last_pixel[2]);
...
TEST_F(YUVFormatConversionTest, I420_To_BGRA32_Consistency)
{
    ...
    ccap::i420ToBgra32(yuv_img.y_data(), yuv_img.y_stride(), yuv_img.u_data(), yuv_img.uv_stride(), yuv_img.v_data(), yuv_img.uv_stride(),
                       bgra_img.data(), bgra_img.stride(), width_, height_);
    ...
    for (int y = 0; y < height_; ++y) {
        ...
        for (int x = 0; x < width_; ++x) {
            for (int c = 0; c < 4; ++c) {
                EXPECT_EQ(bgra_row[x * 4 + c], reference_pixel[c]) << "Inconsistent pixel at (" << x << "," << y << ") channel " << c;
            }
        }
    }
}
...
TEST_F(YUVFormatConversionTest, I420_To_RGBA32_BT709_VideoRange)
{
    ...
    ccap::i420ToRgba32(yuv_img.y_data(), yuv_img.y_stride(), yuv_img.u_data(), yuv_img.uv_stride(), yuv_img.v_data(), yuv_img.uv_stride(),
                       rgba_img.data(), rgba_img.stride(), width_, height_, ccap::ConvertFlag::BT709 | ccap::ConvertFlag::VideoRange);
...
TEST_F(YUVFormatConversionTest, LargeImageSize)
{
    ...
    EXPECT_NO_THROW({
        ccap::i420ToBgr24(yuv_img.y_data(), yuv_img.y_stride(), yuv_img.u_data(), yuv_img.uv_stride(), yuv_img.v_data(),
                          yuv_img.uv_stride(), bgr_img.data(), bgr_img.stride(), large_width, large_height);
    });
}
