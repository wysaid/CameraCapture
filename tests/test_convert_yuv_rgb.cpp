...
TEST_F(YUVToRGBTest, YUV2RGB601VideoRange_KnownValues) {
    ...
    for (const auto& test_case : test_cases) {
        ...
        EXPECT_NEAR(r, test_case.expected_r, 5)
            << "Red mismatch for " << test_case.description << " YUV(" << test_case.y << "," << test_case.u << "," << test_case.v << ")";
        EXPECT_NEAR(g, test_case.expected_g, 5) << "Green mismatch for " << test_case.description;
        EXPECT_NEAR(b, test_case.expected_b, 5) << "Blue mismatch for " << test_case.description;

        // Ensure RGB values are in valid range
        EXPECT_TRUE(PixelTestUtils::isValidRGB(r, g, b)) << "Invalid RGB values for " << test_case.description;
    }
}
...
protected:
    void TestYUVConversion(void (*conv_func)(int, int, int, int&, int&, int&)) {
        ...
        EXPECT_TRUE(PixelTestUtils::isValidRGB(r, g, b))
            << "Invalid RGB(" << r << "," << g << "," << b << ") for YUV(" << y << "," << u << "," << v << ")";
    }
};

TEST_P(YUVConversionParameterizedTest, YUV2RGB601v_ValidRange) { TestYUVConversion(ccap::yuv2rgb601v); }

TEST_P(YUVConversionParameterizedTest, YUV2RGB709v_ValidRange) { TestYUVConversion(ccap::yuv2rgb709v); }

TEST_P(YUVConversionParameterizedTest, YUV2RGB601f_ValidRange) { TestYUVConversion(ccap::yuv2rgb601f); }

TEST_P(YUVConversionParameterizedTest, YUV2RGB709f_ValidRange) { TestYUVConversion(ccap::yuv2rgb709f); }

INSTANTIATE_TEST_SUITE_P(YUVToRGBRangeTest, YUVConversionParameterizedTest, ::testing::ValuesIn(TestDataGenerator::getTestYUVValues()));
    ...
    for (const auto& [y, u, v] : test_yuv) {
        ...
        if (y != 128 || u != 128 || v != 128) {
            EXPECT_TRUE(r1 != r2 || g1 != g2 || b1 != b2)
                << "Video and full range should give different results for YUV(" << y << "," << u << "," << v << ")";
        }
...
TEST_F(YUVToRGBTest, ConsistencyBetweenBT601AndBT709) {
    ...
    for (const auto& [y, u, v] : test_yuv) {
        ...
        EXPECT_TRUE(r601 != r709 || g601 != g709 || b601 != b709)
            << "BT.601 and BT.709 should give different results for YUV(" << y << "," << u << "," << v << ")";
...
TEST_F(YUVToRGBTest, EdgeCases) {
    ...
    for (const auto& edge_case : edge_cases) {
        ...
        EXPECT_TRUE(PixelTestUtils::isValidRGB(r, g, b)) << "BT.601 video: " << edge_case.description;

        ccap::yuv2rgb601f(edge_case.y, edge_case.u, edge_case.v, r, g, b);
        EXPECT_TRUE(PixelTestUtils::isValidRGB(r, g, b)) << "BT.601 full: " << edge_case.description;

        ccap::yuv2rgb709v(edge_case.y, edge_case.u, edge_case.v, r, g, b);
        EXPECT_TRUE(PixelTestUtils::isValidRGB(r, g, b)) << "BT.709 video: " << edge_case.description;

        ccap::yuv2rgb709f(edge_case.y, edge_case.u, edge_case.v, r, g, b);
        EXPECT_TRUE(PixelTestUtils::isValidRGB(r, g, b)) << "BT.709 full: " << edge_case.description;
    }
}
...
TEST_F(YUVToRGBTest, Dual_Implementation_NV12_To_RGBA32) {
    ...
    AVX2TestRunner::runImageComparisonTest(
        [&]() -> TestImage {
            ...
            ccap::nv12ToRgba32(yuv_src.y_data(), yuv_src.y_stride(), yuv_src.uv_data(), yuv_src.uv_stride(), rgba_dst.data(),
                               rgba_dst.stride(), width, height);
...
TEST_F(YUVToRGBTest, Dual_Implementation_NV12_To_BGR24) {
    ...
    AVX2TestRunner::runImageComparisonTest(
        [&]() -> TestImage {
            ...
            ccap::nv12ToBgr24(yuv_src.y_data(), yuv_src.y_stride(), yuv_src.uv_data(), yuv_src.uv_stride(), bgr_dst.data(),
                              bgr_dst.stride(), width, height);
    ...
    AVX2TestRunner::runImageComparisonTest(
        [&]() -> TestImage {
            ...
            ccap::i420ToBgra32(yuv_src.y_data(), yuv_src.y_stride(), yuv_src.u_data(), yuv_src.uv_stride(), yuv_src.v_data(),
                               yuv_src.uv_stride(), bgra_dst.data(), bgra_dst.stride(), width, height);
...
TEST_F(YUVToRGBTest, Dual_Implementation_I420_To_RGBA32) {
    ...
    AVX2TestRunner::runImageComparisonTest(
        [&]() -> TestImage {
            ...
            ccap::i420ToRgba32(yuv_src.y_data(), yuv_src.y_stride(), yuv_src.u_data(), yuv_src.uv_stride(), yuv_src.v_data(),
                               yuv_src.uv_stride(), rgba_dst.data(), rgba_dst.stride(), width, height);
...
TEST_F(YUVToRGBTest, Dual_Implementation_I420_To_BGR24) {
    ...
    AVX2TestRunner::runImageComparisonTest(
        [&]() -> TestImage {
            ...
            ccap::i420ToBgr24(yuv_src.y_data(), yuv_src.y_stride(), yuv_src.u_data(), yuv_src.uv_stride(), yuv_src.v_data(),
                              yuv_src.uv_stride(), bgr_dst.data(), bgr_dst.stride(), width, height);
    ...
    AVX2TestRunner::runImageComparisonTest(
        [&]() -> TestImage {
            ...
            ccap::i420ToRgb24(yuv_src.y_data(), yuv_src.y_stride(), yuv_src.u_data(), yuv_src.uv_stride(), yuv_src.v_data(),
                              yuv_src.uv_stride(), rgb_dst.data(), rgb_dst.stride(), width, height);
